...
#returns a (x,2) matrix that contains valid readings obtained from the middle 6 angles
def getTarget():
    #polarsan returns a (108,2) matrix (distance, angle) # we are only interested in the middle 6 values(51-57)
    
    scan = lidar.polarScan()
    dist = scan[51:57,0]                            #rows 51-57, column 0
    angle = scan[51:57,1]                           #rows 51-57, column 1
    valid = np.where(dist > 0.016)                  # find values 16mm
    myNums = dist[valid]                            # get valid distances
    myAng = angle[valid]                           # get corresponding valid angles
    output = np.vstack((myNums, myAng))             # recombine columns
    n = output.T                                    # transpose the matrix
    return n  
